...
| Retrieval recall for local facts (source-region diversity, relation perspectives, sufficiency-driven focused expansion) | Retrieval | IMPLEMENTED | M3 (corrective 3) | two-stage packet selection (breadth caps per chunk/region → depth), cross-variant RRF, `query-reformulator` 1.1.0 perspective lexicon, `evidence-sufficiency-probe` 1.0.0 pre-generation + post-verification safety net; occupancy reported separately from recall | retrieval recall benchmark (distractor monopolization = 0) | Heuristic lexicons; deep coverage remains M5 |
| Milestone 2 review backlog F1–F23 | Retrieval/API | IMPLEMENTED (closed) | M3 (corrective 3) | see `docs/reviews/milestone-2-backlog-closure.md` | `M2BacklogClosureTest` (15) | none open |
| Product branding (Mnemosyne mark/wordmark/favicon; no starter-kit chrome) | Product completeness | IMPLEMENTED (functional) | M3 (corrective 3) | app mark SVG, wordmark, favicon.svg, meta, sidebar icon audit | `BrandingTest` | Functional identity, not final corporate brand (M6 polish) |
...
| Milestone 2 review backlog (F1–F23) | IMPLEMENTED (closed in M3 corrective pass 3) | Every finding dispositioned with evidence in `docs/reviews/milestone-2-backlog-closure.md`; open items: none. |
